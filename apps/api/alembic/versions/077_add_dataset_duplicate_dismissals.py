...
def upgrade() -> None:
    # `Base.metadata.create_all` on API startup may have already created this
    # table (without recording it in Alembic), so guard against re-creating it.
    inspector = inspect(op.get_bind())
    if not inspector.has_table("dataset_duplicate_dismissals"):
        op.create_table(
    ...
    existing_indexes = {ix["name"] for ix in inspector.get_indexes("dataset_duplicate_dismissals")}
    if "idx_dup_dismissal_project_id" not in existing_indexes:
