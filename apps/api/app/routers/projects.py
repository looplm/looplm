...
from app.project_settings import normalize_project_settings
...
def _mask_settings(settings: dict) -> dict:
    ...
    out = normalize_project_settings(settings)
...
async def update_project(
    ...
    if body.settings is not None:
        merged = normalize_project_settings(project.settings)
        for key, value in body.settings.items():
            ...
            if key in _SECRET_SETTINGS_KEYS and value and "..." in str(value):
                continue
            merged[key] = value
        project.settings = normalize_project_settings(merged)
